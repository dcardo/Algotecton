...
void createMatrix(){
 ...
 //DPoint test_dpoint = new DPoint(0, 0, 0, 20); 

 for (int i = 0; i < xsize+1; i++){
   for (int j = 0; j < ysize+1; j++){
     for (int k = 0; k < zsize+1; k++){
       println ("creating dpoint # " + i + ", " + j + ", " + k);
       dmatrix[i][j][k] = new DPoint((bboxW/xsize)*i, (bboxH/ysize)*j, (-bboxD/zsize)*k, 3);
       //dmatrix[i][j][k] = new DPoint(random(0, bboxW), random(0, bboxH), random(0, bboxD), 3);   
     }
   }
 }
